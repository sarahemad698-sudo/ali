...
export function Dashboard({ roads }: DashboardProps) {
  ...
  const avgOccupancy = Math.round(
    roads.reduce((acc, r) => acc + (r.currentVehicles / r.capacity) * 100, 0) /
      totalRoads
  );

  const EMERGENCY_THRESHOLD = 180;
    ...
    <div className="space-y-6 animate-fadeIn">
        ...
        <Card className="p-6 bg-gradient-to-br from-blue-50 to-blue-100 border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-900 mt-1 text-2xl font-bold">
                {totalRoads}
              </p>
            </div>
        ...
        <Card className="p-6 bg-gradient-to-br from-red-50 to-red-100 border-red-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-red-900 mt-1 text-2xl font-bold">
                {closedGates}
              </p>
            </div>
        ...
        <Card className="p-6 bg-gradient-to-br from-green-50 to-green-100 border-green-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-green-900 mt-1 text-2xl font-bold">
                {openGates}
              </p>
            </div>
        ...
        <Card className="p-6 bg-gradient-to-br from-yellow-50 to-yellow-100 border-yellow-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-yellow-900 mt-1 text-2xl font-bold">
                {avgOccupancy}%
              </p>
            </div>
      ...
      <Card className="p-6">
        <h2 className="text-gray-900 text-xl font-bold mb-6">
          حالة الطرق الحالية (Sensor Data)
        </h2>
        ...
        <div className="space-y-4">
          {roads.map((road) => {
            ...
            // ✅ ONLY WAHAT ROAD AFFECTS ALERT COLORS/EMERGENCY
            const isWahat = road.id === 4 || road.name.includes("الواحات");

            const isEmergency =
              isWahat && !!road.avgGreenTime && road.avgGreenTime < EMERGENCY_THRESHOLD;

            const isHigh = isWahat && occupancyPercent >= 80;
            const isMedium =
              isWahat && occupancyPercent >= 60 && occupancyPercent < 80;
              ...
              >
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span
                        className={`font-mono font-bold ${
                          isEmergency ? "text-blue-600" : "text-gray-900"
                        }`}
                      >
                ...
                {isEmergency && (
                  <div className="bg-blue-100 border border-blue-200 rounded-lg p-3 text-sm text-blue-900 flex items-start gap-2 shadow-sm">
                    ...
                    <span>
                      <strong>حالة طوارئ:</strong> تم فتح الإشارة بالكامل لتسهيل
                      مرور سيارة الإسعاف.
                    </span>
                  </div>
                )}
                ...
                {!isEmergency && isHigh && (
                  <div className="bg-white border border-red-200 rounded-lg p-3 text-sm text-red-800 flex items-start gap-2 shadow-sm">
                    ...
                    <span>
                      <strong>تحذير:</strong> الطريق شبه ممتلئ - نظام الـ ESP32
                      يرسل إشارات لإغلاق البوابة القادمة.
                    </span>
                  </div>
                )}
              </div>
