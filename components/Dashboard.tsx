...
import { RoadData } from '../types';import React from "react";
import { Card } from './ui/card';
import { AlertTriangle, Car, Wind, CheckCircle, XCircle, MapPin, Siren } from 'lucide-react';
import { Progress } from './ui/progress';
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          {roads.map((road) => {
            ...
            const isEmergency = road.avgGreenTime && road.avgGreenTime < 180;
...
export function Dashboard({ roads }: DashboardProps) {
  ...
  const totalRoads = safeRoads.length;
  const closedGates = safeRoads.filter(r => r.gateStatus === 'closed').length;
  const openGates = safeRoads.filter(r => r.gateStatus === 'open').length;

  const avgOccupancy = totalRoads === 0
    ? 0
    : Math.round(
        safeRoads.reduce((acc, r) => {
          const cap = Number(r.capacity) || 0;
          const cars = Number(r.currentVehicles) || 0;
          const pct = cap > 0 ? (cars / cap) * 100 : 0;
          return acc + pct;
        }, 0) / totalRoads
      );
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <Card className="p-6">
        ...
        <div className="space-y-4">
          {safeRoads.map((road) => {
            const cap = Number(road.capacity) || 0;
            const cars = Number(road.currentVehicles) || 0;
            const occupancyPercent = cap > 0 ? (cars / cap) * 100 : 0;

            const isWahat = road.id === 4; // ✅ طريق الواحات فقط
            const isEmergency = isWahat && !!road.avgGreenTime && road.avgGreenTime < 180;
            const isHigh = isWahat && occupancyPercent >= 80;
            const isMedium = isWahat && occupancyPercent >= 60 && occupancyPercent < 80;
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-3">
                  <div>
                    ...
                    <Progress
                      value={Number.isFinite(occupancyPercent) ? occupancyPercent : 0}
                      className={`h-3 ${
                        !isWahat
                          ? '[&>div]:bg-green-600'
                          : isEmergency 
                  ...
                  <div>
                    ...
                    <Progress
                      value={(Number(road.avgGreenTime || 0) * 2)} // Assuming 50s max for viz
                      className={`h-3 ${
                         isEmergency ? '[&>div]:bg-blue-600' : '[&>div]:bg-green-600'
                      }`}
                    />
